...
dcel_create(int* num_vertcs, point_ptr ** vertcs, int* num_dcel, 
    ...
    for (i = 0; i < *num_vertcs; i++) {
        ...
        *location = (edge_ptr*)realloc(*location, sizeof(edge_ptr) 
        ...
        // Assigning index of start vertex
        (*location)[*num_dcel]->idx_strt = i;

        // Assigning index of end vertex
        if (i + 1 == *num_vertcs) {
        ...
        // Assigning pointer to previous DCE and pointer to next DCE
        ...
        // Assigning point to other half edge (in other directon)
        (*location)[*num_dcel]->otr_edge = NULL;

        // Assigning the index of its corresponding face
        (*location)[*num_dcel]->corr_face = FACE;
        
        // Assigning the index of the edge it forms a part of
